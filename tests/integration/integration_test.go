...
func checkTaskBatch(
	...
	aggBatchIdx, aggTaskIdx, err := submitTasks(batch.tasks)
	if err != nil {
		t.Fatalf("failed to submit tasks - %s", err)
	}
	assert.Equal(t, batch.index, aggBatchIdx)
	assert.Equal(t, batch.lastTaskIndex, aggTaskIdx)
	...
	case onchainBatch := <-batchCh:
		...
		assert.Equal(t, batch.index, onchainBatch.Batch.Index)
		assert.Equal(t, batch.index+1, nextBatchIdx)
		...
		assert.Equal(t, batchMerkleRoot, onchainBatch.Batch.MerkeRoot)
