#!/bin/bash

cli setup-operator \
    --config tests/nodes/operators/1/config-devnet.yaml \
    --bls-password hG6zoZiGJOf6okxSO1G5 \
    --ecdsa-password KRnd3uPtKuCl1NkDWzdE \
    --strategy-address 0x09635F643e140090A9A8Dcd712eD6285858ceBef \
    --strategy-deposit-amount 10

OPERATOR_BLS_KEY_PASSWORD=hG6zoZiGJOf6okxSO1G5 OPERATOR_ECDSA_KEY_PASSWORD=KRnd3uPtKuCl1NkDWzdE operator --config tests/nodes/operators/1/config-devnet.yaml